...
Execute a tactical spec **one step at a time**. Read `PLAN/<ID>_<slug>/INDEX.md` +
...
5. **Pre-edit guards:** read-only zone → abort; file over ~`500` lines and not yet backed
   up → back up to `temp/` first; projected size past the budget → abort and split;
...
- **Phase done criteria:** run each checkbox. For the phase's verification rung, run the
  narrowest meaningful check the phase names (per `doc/VALIDATION.md`) - a compile / type-check,
  a targeted test, or `dotnet build -c Release` only when the phase touches packaging, resources, or wiring.
...
- Anti-slop applies while implementing (`doc/CODE_QUALITY.md`).
- Dead-weight hygiene: when a step orphans code/resources/deps, delete the remnant in the
  ...
  artifact, grep `PLAN/` for active-ticket scaffolding and do not remove another
